        ...
        @foreach($items as $metadata_key => $metadata_value)
            <div class="mb-10">
                ...
                @if($metadata_value['type'] === 'text')
                    <input type="text" name="metadata[{{ $metadata_key }}]" class="form-control" placeholder="{{ $metadata_value['placeholder'] }}" value="{{ $values[$metadata_key] ?? '' }}">
                @endif
                @if($metadata_value['type'] === 'number')
                    <input type="number" name="metadata[{{ $metadata_key }}]" class="form-control" placeholder="{{ $metadata_value['placeholder'] }}" value="{{ $values[$metadata_key] ?? '' }}">
                @endif
